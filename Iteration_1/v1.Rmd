...
Testing to see if DBscan will work, but unfortunately it didnt
```{r}
# https://www.youtube.com/watch?v=m9pcY6iqIZY
...
kNNdistplot(metadata_all_summary[ , c(9:22)], k = 5)
# Compute DBSCAN using fpc package
library("fpc")
set.seed(123)
dbscan <- fpc::dbscan(metadata_all_summary[ , c(9:22)], eps = 350, MinPts = 30)

# Plot DBSCAN results
library("factoextra")
fviz_cluster(dbscan, data = metadata_all_summary[ , c(9:22)], stand = FALSE,
...
qplot(metadata_summary_test$actual_ftp_perc,
      geom="histogram",
      binwidth = 0.01,  
      main = "Histogram for actual_FTP_perc relative to best max 20min number", 
...
library(ggplot2)
# Building histogram to test each variable to see if it follows a normal distribution
...
# need to do more research on how to remove outliers based on a formula and move this up to a section closer to the top, in the data summary
# Next step is to check for correlation
...
```{r}
names(lmModel_1)
#The above vector presents the names of the object that constitute the model object. Here, fitted values are the predicted values. Now, we will use these values to generate the rmse values.(reword straight from the text)
```
```{r}
...
```{r}
# confirm assumptions of a linear regression
...
# 2. There should be no heteroscedasticity – This means that the variance of error terms should be constant. We shall not see any patterns when we draw a plot between residuals and fitted values. A straight red line closer to the zero value represents that we do not have heteroscedasticity problem in our data. #see first diagram heteroscedasticity
```
...
# create dataset for vif test
# need to exclude 
lr_data_vif <- subset(lr_data_sig, select = -c(max_critical_power_60m))
vif_func(lr_data_sig[,1:8])

# based on VIF results it makes sense to remove either avg_power_all_rides or avg_coggan_np

# https://blog.minitab.com/en/understanding-statistics/handling-multicollinearity-in-regression-analysis#:~:text=A%20VIF%20between%205%20and,poorly%20estimated%20due%20to%20multicollinearity.
```
...
test_1$predicted_max_critical_power_60m <- predict(lmModel_1, test_1)
...
test_1$predicted_max_critical_power_60m <- predict(lmModel_1, test_1)
actual <- test_1$max_critical_power_60m
preds <- test_1$predicted_max_critical_power_60m
rss <- sum((preds - actual) ^ 2)
tss <- sum((actual - mean(actual)) ^ 2)
rsq <- 1 - rss/tss
rsq
```
The final step would be to add the eFTP value to the test_1 data to compare the FTP, predicted FTP and eFTP
```{r}
final_test <- cbind(test_1, test$max_critical_power_20m, test$max_critical_power_20m*0.95)
library(dplyr)
final_test <- rename(final_test, max_critical_power_20m = V2)
final_test <- rename(final_test, eFTP = V3)
str(final_test$V2)
```
